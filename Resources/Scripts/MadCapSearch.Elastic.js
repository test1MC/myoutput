(function () {
    ...
    function RunGeneralSearch(input, options) {
        var search = WebService("search/query?index=" + index + "&input=" + input + "&page=" + options.content.pageIndex + "&pageSize=" + options.content.pageSize, "", "Search failed.")
            .then(function (response) {
                ...
                if (response.microContent) {
                    var microContent = response.microContent;
                    microContent.html = _helpSystem.ProcessMicroContentXhtml(microContent.html);
                    microContent.link = GetLinkUrl(microContent.link).FullPath;
                    searchResults.microContent = microContent;
                }
                if (results.length > 0) {
                    ...
                    for (var i = 0; i < results.length; i++) {
                        ...
                        var searchResult = new Search.SearchResult(score, rank, unescape(title), linkUrl.FullPath, unescape(abstractText), true);
    ...
    function selectItem($item) {
        var selectedId = "selected-item";
        $item.addClass("auto-result-selected");
        $item.attr({
            'id': selectedId,
            'aria-selected': 'true'
        });
        $('.search-field').attr("aria-activedescendant", selectedId);
        ...
        if (searches.length > 0) {
            ...
            for (var i = 0; i < searches.length; i++) {
                ...
                if (isHistoryResult) {
                    ...
                    listItem.setAttribute("aria-label", MadCap.Accessibility.getElasticSearchHistoryLabel(search.input, searchField));
                }
                else {
                    ...
                    listItem.setAttribute("data-mc-input", search.title);
                    listItem.setAttribute("aria-label", MadCap.Accessibility.getElasticSearchContentLabel(search.title, searchField));
                }
    ...
    function DeleteUserSearch($el, input) {
        WebService("search/deletepersonalasync?index=" + index + "&input=" + input, "", "Delete failed.")
            .then(function (resp) {
                if (resp)
                    $el.remove();
            });
    ...
    var ElasticsearchOn = function () {
        function AddSelectionEvents() {
            $(document).on("mouseenter mouseleave", ".search-auto-complete li", function () {
            ...
            $(".search-field").keydown(function (e) {
                var key = e.keyCode;
                var moveDown = key == 40;
                var moveUp = key == 38; // up
                var tab = key == 9;
                var submit = key == 13; // enter

                var $list = $(".search-auto-complete"),
